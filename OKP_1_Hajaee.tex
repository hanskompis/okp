...
Merkkiliteraali ilmaistaan \$ -merkin perään kirjoitetulla merkillä tai ohjausmerkillä (escape secuence) : 
\begin{verbatim}
 $R $s $$ $\\
\end{verbatim}
...
Atomiliteraalit ovat Erlangin erikoisuus. Ne ovat merkkijonoliteraalivakioita, joiden arvo on atomi itse. Atomeihin ei voi sitoa mitään arvoa. Esimerkiksi atomi 
\begin{verbatim}
 auto
\end{verbatim}
tarkoittaa autoa, eikä mitään muuta. Atomi alkaa pienellä kirjaimella, \textunderscore :lla tai \makeatletter @ \makeatother -merkillä. Jos atomin haluaa aloittaa muulla tavoin, tulee se ympäröidä \' -merkeillä.  

Muuttujan nimen tulee alkaa isolla kirjaimella,  \makeatletter @ \makeatother :llä, tai \textunderscore :lla:
\begin{verbatim}
 Data I 
\end{verbatim}
Koska mitkään varatut sanat, operaattorit tai avainsanat eivät ala isolla kirjaimella, tulkintavirhe ei ole mahdollinen.  

Universaalimerkkinä käytetään '\textunderscore' -merkkiä. Sen voidaan ajatella olevan mikä tahansa merkki. Hahmonsovituksessa tähän merkkiin vertautuu mikä tahansa arvo totena.  
...
D-kielessä voidaan muuttuja määritellä myös auto, jolloin muuttujan tyyppi päätellään käänösvaiheessa. Alla on esimerkki tällaisesta.
\begin{verbatim}
import std.stdio;
